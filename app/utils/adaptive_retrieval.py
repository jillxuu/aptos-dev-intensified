...
from app.utils.vector_store_utils import (
    batch_similarity_search_with_score,
    similarity_search_with_batch_support,
)
...
async def generate_retrieval_queries(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if "retrieval_queries" not in result or not isinstance(
            result["retrieval_queries"], list
        ):
        ...
        logger.info(
            f"[ADAPTIVE-RAG] Generated {len(result['retrieval_queries'])} retrieval queries in {elapsed:.2f}s"
        )
        return result
...
async def analyze_follow_up_needs(
    query: str, initial_results: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if "follow_up_queries" not in result or not isinstance(
            result["follow_up_queries"], list
        ):
        ...
        logger.info(
            f"[ADAPTIVE-RAG] Follow-up analysis completed in {elapsed:.2f}s, generated {query_count} follow-up queries"
        )
        return result
    except Exception as e:
...
async def execute_query(
    ...
    elapsed = time.time() - start_time
    logger.info(
        f"[ADAPTIVE-RAG] {query_type.capitalize()} query completed in {elapsed:.2f}s, found {len(processed_results)} results"
    )
...
async def execute_batch_queries(
    ...
    start_time = time.time()
    logger.info(
        f"[ADAPTIVE-RAG] Starting batch {query_type} queries: {len(queries)} queries"
    )
    ...
    logger.info(
        f"[ADAPTIVE-RAG] Batch similarity search completed in {batch_search_time:.2f}s"
    )
    ...
    for doc, score in all_docs_with_scores:
        ...
        for related_id in related_ids:
            if related_id in chunk_map:
                ...
                if (
                    similarity >= 0.7
                ):  # Using SIMILARITY_THRESHOLD value from topic_chunks
    ...
    logger.info(
        f"[ADAPTIVE-RAG] Batch {query_type} queries completed in {total_time:.2f}s"
    )
    logger.info(f"[ADAPTIVE-RAG] PERFORMANCE BREAKDOWN:")
    logger.info(
        f"[ADAPTIVE-RAG] - Batch similarity search: {batch_search_time:.2f}s ({(batch_search_time/total_time)*100:.1f}%)"
    )
    logger.info(
        f"[ADAPTIVE-RAG] - Results processing: {process_time:.2f}s ({(process_time/total_time)*100:.1f}%)"
    )
    logger.info(
        f"[ADAPTIVE-RAG] Found {result_count} unique results across {len(queries)} queries"
    )
...
async def adaptive_multi_step_retrieval(
    ...
    logger.info(
        f"[ADAPTIVE-RAG] Starting multi-step retrieval process for query: {query}"
    )
    ...
    logger.info(
        f"[ADAPTIVE-RAG] Prepared {len(all_queries)} queries in {queries_elapsed:.2f}s"
    )
    ...
    if len(all_queries) > 1:
        ...
        logger.info(
            f"[ADAPTIVE-RAG] Executing primary retrieval using batched search for {len(all_queries)} queries"
        )
        primary_results = await execute_batch_queries(
            all_queries, vector_store, enhanced_chunks, k, "primary"
        )
        ...
        primary_result_pairs = await execute_query(
            all_queries[0], vector_store, enhanced_chunks, k, "primary"
        )
    ...
    logger.info(
        f"[ADAPTIVE-RAG] Primary retrieval completed in {primary_elapsed:.2f}s, found {len(all_results)} unique chunks"
    )
    ...
    if not follow_up_analysis.get("is_complete", True) and follow_up_queries:
        ...
        if follow_up_queries:
            ...
            if len(follow_up_queries) > 1:
                logger.info(
                    f"[ADAPTIVE-RAG] Executing follow-up retrieval using batched search for {len(follow_up_queries)} queries"
                )
                follow_up_results = await execute_batch_queries(
                    follow_up_queries, vector_store, enhanced_chunks, k, "follow_up"
                )
                ...
                logger.info(
                    f"[ADAPTIVE-RAG] Executing single follow-up query using standard search"
                )
                follow_up_result_pairs = await execute_query(
                    follow_up_queries[0], vector_store, enhanced_chunks, k, "follow_up"
                )
            ...
            logger.info(
                f"[ADAPTIVE-RAG] Follow-up queries executed in {follow_up_queries_elapsed:.2f}s, found {new_chunks_count} new chunks"
            )

    follow_up_elapsed = time.time() - follow_up_start_time
    logger.info(
        f"[ADAPTIVE-RAG] Total follow-up processing completed in {follow_up_elapsed:.2f}s"
    )
    ...
    logger.info(
        f"[ADAPTIVE-RAG] - Primary retrieval ({len(all_queries)} queries): {primary_elapsed:.2f}s"
    )
    ...
    logger.info(
        f"[ADAPTIVE-RAG] Retrieved {len(combined_results)} unique chunks across all queries"
    )
