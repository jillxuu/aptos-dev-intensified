...
async def batch_similarity_search_with_score(
    ...
    logger.info(
        f"[VECTOR-STORE] Starting batch similarity search for {len(queries)} queries"
    )
    ...
    try:
        ...
        logger.info(
            f"[VECTOR-STORE] FAISS batch search completed in {search_time:.4f}s"
        )
        ...
        if not docstore_dict:
            logger.warning(
                "[VECTOR-STORE] Could not access docstore._dict, falling back to individual lookups"
            )
        ...
        if hasattr(vector_store, "index_to_docstore_id"):
            logger.info(
                "[VECTOR-STORE] Using docstore_id mapping for document retrieval"
            )
            ...
            for j, (score, idx) in enumerate(zip(scores, indices)):
                ...
                try:
                    ...
                    if hasattr(vector_store, "index_to_docstore_id"):
                        ...
                        if not docstore_id:
                            logger.warning(
                                f"[VECTOR-STORE] No docstore ID found for index {idx}"
                            )
                            continue
                    else:
                        docstore_id = str(idx)
                    ...
                    if not doc:
                        logger.warning(
                            f"[VECTOR-STORE] Document with ID {docstore_id} not found in docstore"
                        )

                        # Try to get document from the vector store's document store
                        if hasattr(vector_store, "_documents") and isinstance(
                            vector_store._documents, list
                        ):
                            # Try to find document in the cached list
                            for cached_doc in vector_store._documents:
                                if str(idx) == getattr(
                                    cached_doc, "id", None
                                ) or docstore_id == getattr(cached_doc, "id", None):
                                    doc = cached_doc
                                    break
                        ...
                        if not doc:
                            logger.error(
                                f"[VECTOR-STORE] Could not retrieve document for index {idx} / docstore_id {docstore_id}"
                            )
                            continue
                    ...
                    if not isinstance(doc, Document):
                        ...
                        if isinstance(doc, str):
                            logger.warning(
                                f"[VECTOR-STORE] Got string instead of Document, creating fallback Document"
                            )
                            doc = Document(page_content=doc, metadata={})
                        else:
                            logger.warning(
                                f"[VECTOR-STORE] Expected Document object but got {type(doc)}, skipping"
                            )
                            continue
                    ...
                    logger.error(
                        f"[VECTOR-STORE] Error processing result {j} for query {i}: {str(e)}"
                    )
                    continue
        ...
        logger.info(
            f"[VECTOR-STORE] Results processing completed in {process_time:.4f}s"
        )

        # Log the number of results found
        result_count = sum(len(results) for results in all_results)
        logger.info(
            f"[VECTOR-STORE] Found {result_count} documents across {len(queries)} queries"
        )
        ...
        logger.info(
            f"[VECTOR-STORE] - Embedding: {embed_time:.4f}s ({(embed_time/total_time)*100:.1f}%)"
        )
        logger.info(
            f"[VECTOR-STORE] - FAISS search: {search_time:.4f}s ({(search_time/total_time)*100:.1f}%)"
        )
        logger.info(
            f"[VECTOR-STORE] - Results processing: {process_time:.4f}s ({(process_time/total_time)*100:.1f}%)"
        )
...
async def similarity_search_with_batch_support(
    ...
    if queries and len(queries) > 1:
        logger.info(
            f"[VECTOR-STORE] Dispatching to batch search for {len(queries)} queries"
        )
        ...
        for query_results in all_results:
            for doc, score in query_results:
                ...
                if not isinstance(doc, Document):
                    logger.warning(
                        f"[VECTOR-STORE] Unexpected result type: {type(doc)}, skipping"
                    )
                    continue
                ...
                if not doc_id and hasattr(doc, "page_content"):
                    ...
                    content_sample = (
                        doc.page_content[:50] if len(doc.page_content) > 0 else "empty"
                    )
        ...
        logger.info(
            f"[VECTOR-STORE] Found {len(results)} unique documents after merging results"
        )
    ...
    # For single query, use the standard search
        ...
        try:
            ...
            logger.error(
                f"[VECTOR-STORE] Error during standard similarity search: {str(e)}"
            )
            return []
